...
class S_Page
{
    ...
    var $isNextPage = true;  //是否显示上一页下一页
    var $isFirstPage = false;//是否显示首页尾页
   ...
   public function set($key,$val){
    ...
    function Calculation_Page_Num($total) //计算页码，传入的数据总条数
    {
        $this->page_num = ceil($total / $this->length);
        return $this->page_num;
    }

    function Calculation_Min_Max($act_page = 1)  //计算最大和最小可选择的页码
    {
        // 定义左右偏移量
        $py_left = 0;
        $py_right = 0;
        ...
        $gd_right = 0;
        // 判断是否需要分组
        if (($this->page_num - $this->page_max) <= 0) //如果总页码数减去最大页数小于等于零，那么不需要分组（即能显示出所有页码）
        {
         // 不需要分组
            $bj_left = 1;//左边界是1
            $bj_right = $this->page_num;//右边界是总页码数
        } else {
           // 要进行分组
           // 判断容量的奇偶
            $tmp = $this->page_max % 2;
            if ($tmp === 1) {
            // 奇数
                $py_left = $py_right = ($this->page_max - 1) / 2; //如果是奇数，那么当前页码的左右页码数就是总页数减一除二
            } else {
            // 偶数
                $py_left = $this->page_max / 2 - 1; //如果是偶数，就按照总页码数进行运算
     ...
     // 主方法
    public function page($total, $act_page, $url, $param)
    {   //传入总条数，当前活动页码，当前url，当前参数
        $page_num = $this->Calculation_Page_Num($total);
        $arr_min_max = $this->Calculation_Min_Max($act_page);

        if (!preg_match("([?|&]$param=)", $url)) {  //使用正则解析url
            $url = strpos($url, "?") === false ? $url . "?" : $url . "&";
            $url = $url . "$param=0";
        }

        if ($act_page > $page_num)  //如果当前活动页大于总页数，就让二者相等
        {
            $act_page = $page_num;
        }

        // 用正则把url改成正规的
        $url = preg_replace('('.$param . '=[0-9]+)', $param . '=0', $url);
        $res = array();
        $d = 0;
